"""
Benchmark 180-Day Lead Forecasting Model (STATIONARY TARGET)
Model predicts PERCENTAGE GROWTH RATE, then reconstructs absolute stars
...
# --- Config ---
VAL_PATH = 'val_dataset_lead180d_stationary.parquet'
MODEL_PATH = 'lgbm_github_lead180d_stationary.pkl'
...
def load_resources():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(f"Model file not found: {MODEL_PATH}\nRun train_model_lead180d_stationary.py first")
...
def get_suitable_repos(lf, min_days=200):
...
def prepare_features(df, model):
    """Extract features for prediction (no lead columns, no total_stars)"""
...
def run_benchmark():
    ...
    repo_names = get_suitable_repos(lf, min_days=200)
    ...
    for idx, repo_name in enumerate(repo_names):
        ...
        # Plot actual stars 180 days ahead
        ax.plot(dates, actual_future_stars, label=f'Actual Stars (t+{FORECAST_HORIZON}d)', 
                color='black', linewidth=2)
        
        # Plot predicted stars 180 days ahead (reconstructed from growth rate)
    ...
    print(f"\n✅ STATIONARY MODEL: Predicts growth rate (scale-invariant)")
    print(f"✅ Reconstruction: predicted_stars = current_stars × (1 + predicted_growth)")
    print(f"✅ NO FUTURE DATA LEAKAGE - Uses only current/past features")
    print(f"\n💡 Benefits of stationary target:")
    print(f"   - Scale-invariant: Works equally well for small and large repos")
    print(f"   - Removes trend: Model learns growth patterns, not absolute levels")
    print(f"   - Better generalization: Growth rate is more stable across repo sizes")
