"""
Benchmark 30-Day Lead Forecasting Model
Direct forecast: Predict stars 30 days ahead using only current/past data
...
# --- Config ---
VAL_PATH = 'val_dataset_lead30d.parquet'
MODEL_PATH = 'lgbm_github_lead30d.pkl'
FORECAST_HORIZON = 30
...
def get_suitable_repos(lf, min_days=60):
...
def run_benchmark():
    ...
    repo_names = get_suitable_repos(lf, min_days=60)
    ...
    for idx, repo_name in enumerate(repo_names):
        ...
        # Remove rows with null targets (last 30 days won't have lead values)
        ...
        mape = (np.abs(errors) / actual_stars * 100).mean()
    ...
    for stat in all_stats:
        print(f"{stat['repo']:<30} {stat['samples']:>10,} {stat['mae']:>10.1f} "
              f"{stat['rmse']:>10.1f} {stat['mape']:>7.1f}%")
    ...
    print('-'*80)
    print(f"{'AVERAGE':<30} {'':<10} {avg_mae:>10.1f} {'':>10} {avg_mape:>7.1f}%")
    print('='*80)
